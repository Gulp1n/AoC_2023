...
data class Game(val id: Int, val matches: List<Match>)
data class Match(val green: Int = 0, val blue: Int = 0, val red: Int = 0)

fun getId(string: String): Int {
    val regex = """(?<=Game\s)(.*)(?=:)""".toRegex()

    val res = regex.find(string) ?: TODO("WTF IS GOING ON")

    return res.value.toInt()
}

fun getGame(string: String): Game {
    val regex = """(?<=[:;])(.*?)(?=[;$])""".toRegex()

    val res = regex.findAll(string).toMutableList()

    val list: MutableList<Match> = mutableListOf()

    res.forEach {
        list.add(getMatches(it.value))
    }

    return Game(getId(string), list.toList())
}

fun getMatches(string: String): Match {
    val rGreen = """([0-9]*)(?=\sgreen)""".toRegex()
    val rBlue = """([0-9]*)(?=\sblue)""".toRegex()
    val rRed = """([0-9]*)(?=\sred)""".toRegex()

    val resGreen = rGreen.find(string)
    val resBlue = rBlue.find(string)
    val resRed = rRed.find(string)
    ...
    if (resGreen != null){
        green += resGreen.value.toInt()
    }
    if (resBlue != null){
        blue += resBlue.value.toInt()
    }
    if (resRed != null){
        red += resRed.value.toInt()
    }

    return(Match(green, blue, red))
}
